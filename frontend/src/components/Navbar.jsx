...
import { Activity, MapPin, AlertOctagon, RefreshCw, LogOut, UserCheck } from 'lucide-react';

export default function Navbar({ activeRole, setActiveRole, currentUser, onLogout, onTriggerEmergency, onResetRoute }) {
  return (
        ...
        <div className="flex items-center gap-3">
          ...
          {currentUser && (
            <div className="hidden md:flex items-center gap-2 px-3 py-1 bg-slate-950 rounded-xl border border-slate-800 text-xs">
              ...
              <span className="text-[10px] text-slate-500 uppercase font-mono">({currentUser.role.replace('_', ' ')})</span>
            </div>
          )}
